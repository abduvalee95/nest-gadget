...
export class SocketGateway implements OnGatewayInit {
	...
	public afterInit(server: Server) {
		// bu SocketGateway tayor bolganda ishga tushadi
		// Socket Module Serverda paydo bolganda log ishga tushadi
	...
	const authMember = await this.retrieveAuth(req)
		...
		this.logger.verbose(`~~~~ Clients Connetcted Total: [${this.summaryClient}] ~~~~`); //Faqatgina murojatchiga yuboriladi 
 ...
 public	handleDisconnect(client: WebSocket) {
		...
		const clientNick: string = authMember?.memberNick ?? 'Guest;';
		this.logger.verbose(`~~~~ Disconnected Left [${clientNick} & Total: [${this.summaryClient}] ~~~~`);
	...
	public async handleMessage(client: WebSocket, payload: string): Promise<void> {
		...
		if (this.messagesList.length > 5) this.messagesList.splice(0, this.messagesList.length - 5); //eng ohirgi 5 ta messageni saqlemiz 
